module Danger
  module Changelog
    ...
    class ChangelogEntryLine < ChangelogLine
      CHANGELOG_ENTRY_TRAILING_REGEX = "[\`[:upper:]].* \- \[\@[\w\d\-\_]+\]\(https:\/\/github\.com\/.*[\w\d\-\_]+\).$".freeze
      CHANGELOG_ENTRY_LEADING_REGEX = "^\*\s".freeze

      def valid?
        return true if line =~ /#{CHANGELOG_ENTRY_TRAILING_REGEX}#{CHANGELOG_ENTRY_TRAILING_REGEX}/
      ...
      def self.validates_as_changelog_line?(line)
        line =~ /#{CHANGELOG_ENTRY_TRAILING_REGEX}/
      end
